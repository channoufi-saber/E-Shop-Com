...
export class ProductService {
  ...
  async update(id: string, updateProductDto: UpdateProductDto) {
    const {
      title, description, imgUrl1, price, quantity, size, color, shippings,
      sex, brands, category, subcategory
    } = updateProductDto;
    const findProduct = await this.productRepository.findOneBy({ id: id });
    if (!findProduct) {
      throw new HttpException('No product found!!', HttpStatus.NOT_FOUND)
    ...
    const findProductFinal = await this.productRepository.findOneBy({ id: id });
    return findProductFinal
